...
FreezeScreen {
    ...
    function processScreenshot(x, y, width, height) {
        ...
        screenshotProcess.command = ["sh", "-c", `magick "${tempPath}" -crop ${scaledWidth}x${scaledHeight}+${scaledX}+${scaledY} "${outputPath}" && ` + `satty --filename "${outputPath}" && ` + `rm "${tempPath}"`];
                        ...
                        background: Rectangle {
                            ...
                            color: {
                                if (mode === modelData.mode)
                                    return Qt.rgba(0.7, 0.7, 0.7, 0.5);
